...
const databasePath =
  process.env.USAGE_HISTORY_DATABASE_PATH ||
  `${import.meta.dir}/data/usage-history.sqlite`;
...
let storageCached: ReturnType<typeof storageInventory> | undefined;
let storageCachedAt = 0;

const storageCacheMs = 5 * 60 * 1000;
const storageCategoryDefinitions = [
  {
    name: "Workspace",
    path: "/home/workspace",
    description: "Your projects, documents, and source code",
    color: "#2d7052",
  },
  {
    name: "Zo data",
    path: "/home/.z",
    description: "Zo workspaces, caches, and internal data",
    color: "#8ebf4c",
  },
  {
    name: "OpenClaw",
    path: "/root/.openclaw",
    description: "Agents, sessions, plugins, and generated data",
    color: "#d4a247",
  },
  {
    name: "Developer tools",
    path: "/root/.npm",
    description: "Package caches and developer tooling",
    color: "#cc775c",
  },
  {
    name: "Tool cache",
    path: "/root/.cache",
    description: "Build, browser, and application caches",
    color: "#8d7ab8",
  },
];
...
database.run(
  "CREATE INDEX IF NOT EXISTS application_traffic_samples_at ON application_traffic_samples(at)",
);
...
function manifestApplications(path: string): Record<string, string> {
  ...
  if (
    ...
    typeof manifest.applications !== "object" ||
    Array.isArray(manifest.applications)
  ) {
    throw new Error('Manifest must contain an "applications" object');
  }
  return Object.fromEntries(
    Object.entries(manifest.applications).filter(
      ([service, label]) =>
        service.trim() && typeof label === "string" && label.trim(),
    ),
...
function loadApplicationNames() {
  ...
  try {
    ...
    console.warn(
      `Unable to load application manifest override at ${overridePath}: ${error}`,
    );
    return names;
  }
}
...
function readCpu() {
  ...
  try {
    ...
    return Math.max(
      0,
      Math.min(100, (100 * (usage - previous.total)) / available),
    );
  } catch {
    // Fall back to procfs on hosts without the cgroup CPU accounting counter.
  }
  const values = readFileSync("/proc/stat", "utf8")
    .split("\n")[0]
    .trim()
    .split(/\s+/)
    .slice(1)
  ...
  return Math.max(
    ...
    Math.min(
      100,
      100 * (1 - (idle - previous.idle) / (total - previous.total)),
    ),
  );
}
...
function readNetworkTotals() {
  return readFileSync("/proc/net/dev", "utf8")
    ...
    .reduce(
      (sum, parts) => ({
        received: sum.received + number(parts[1]),
        sent: sum.sent + number(parts[9]),
        receivedPackets: sum.receivedPackets + number(parts[2]),
        sentPackets: sum.sentPackets + number(parts[10]),
      }),
...
function recordBandwidth() {
  ...
  const exists = database
    .query("SELECT 1 FROM network_samples WHERE at = ?")
    .get(at);
  if (exists) return;
  ...
  const previous = database
    .query(
      "SELECT received_bytes, sent_bytes, received_packets, sent_packets FROM network_samples ORDER BY at DESC LIMIT 1",
    )
    .get() as {
  ...
  const receivedDelta =
    previous && current.received >= previous.received_bytes
      ? current.received - previous.received_bytes
      : 0;
  const sentDelta =
    previous && current.sent >= previous.sent_bytes
      ? current.sent - previous.sent_bytes
      : 0;
  const receivedPacketsDelta =
    previous && current.receivedPackets >= previous.received_packets
      ? current.receivedPackets - previous.received_packets
      : 0;
  const sentPacketsDelta =
    previous && current.sentPackets >= previous.sent_packets
      ? current.sentPackets - previous.sent_packets
      : 0;
  ...
  if (Date.now() - historyPrunedAt > 24 * 60 * 60 * 1000) {
    database
      .query("DELETE FROM network_samples WHERE at < ?")
      .run(Date.now() - 30 * 24 * 60 * 60 * 1000);
    database
      .query("DELETE FROM network_daily WHERE day < ?")
      .run(singaporeDate(Date.now() - 365 * 24 * 60 * 60 * 1000));
    database
      .query("DELETE FROM application_traffic_samples WHERE at < ?")
      .run(Date.now() - 30 * 24 * 60 * 60 * 1000);
    database
      .query("DELETE FROM application_traffic_daily WHERE day < ?")
      .run(singaporeDate(Date.now() - 365 * 24 * 60 * 60 * 1000));
...
function bandwidthHistory() {
  ...
  const connectionOwners = [
    ...snapshot().connections.reduce((owners, connection) => {
      owners.set(
        connection.application,
        (owners.get(connection.application) || 0) + 1,
      );
      return owners;
  ...
  try {
    ...
    if (!Array.isArray(payload.samples) || payload.samples.length > 64) {
      return Response.json(
        { error: "samples must be an array of up to 64 entries" },
        { status: 400 },
      );
    }
    ...
    for (const sample of payload.samples) {
      ...
      const rawApplication =
        typeof value.application === "string" ? value.application.trim() : "";
...
function applicationTrafficHistory(application?: string) {
  ...
  return {
    ...
    processes: application
      ? live?.processes.filter((process) => process.application === application)
      : [],
    connections: application
      ? live?.connections.filter(
          (connection) => connection.application === application,
        )
      : [],
    detailedRetentionDays: 30,
    dailyRetentionDays: 365,
...
function readNetwork() {
  ...
  if (!previous || now <= previous.at)
    return { ...totals, down: 0, up: 0, downPackets: 0, upPackets: 0 };
  const seconds = (now - previous.at) / 1000;
  return {
    down: Math.max(0, (totals.received - previous.received) / seconds),
    up: Math.max(0, (totals.sent - previous.sent) / seconds),
    downPackets: Math.max(
      0,
      (totals.receivedPackets - previous.receivedPackets) / seconds,
    ),
    upPackets: Math.max(
      0,
      (totals.sentPackets - previous.sentPackets) / seconds,
    ),
    ...totals,
  };
}
...
function readDisk() {
  const output =
    new TextDecoder()
      .decode(Bun.spawnSync(["df", "-B1", "/"]).stdout)
      .trim()
      .split("\n")[1]
      ?.trim()
      .split(/\s+/) || [];
...
function directoryUsage(paths: string[]) {
  const existing = [...new Set(paths)].filter((path) => existsSync(path));
  if (!existing.length) return new Map<string, number>();
  const output = new TextDecoder().decode(
    Bun.spawnSync(["/usr/bin/du", "-sx", "-B1", ...existing]).stdout,
  );
  return new Map(
    output
      .split("\n")
...
function storageInventory() {
  const locationDefinitions = [
    ...storageCategoryDefinitions,
    {
      name: "Garden of Zo",
      path: "/home/workspace/Start/garden-of-zo",
      description: "Hosted Zo applications",
    },
    {
      name: "Hackathons",
      path: "/home/workspace/hackathon",
      description: "Hackathon projects and assets",
    },
    {
      name: "Workspace GitHub",
      path: "/home/workspace/Github",
      description: "Workspace GitHub clones",
    },
    {
      name: "Zo workspaces",
      path: "/home/.z/workspaces",
      description: "Zo conversation workspaces",
    },
    {
      name: "OpenClaw agents",
      path: "/root/.openclaw/agents",
      description: "Agent sessions and state",
    },
    {
      name: "OpenClaw packages",
      path: "/root/.openclaw/npm",
      description: "Installed OpenClaw plugins",
    },
  ];
  const sizes = directoryUsage(
    locationDefinitions.map((location) => location.path),
  );
  const categories = storageCategoryDefinitions
    .map((category) => ({ ...category, bytes: sizes.get(category.path) || 0 }))
    .filter((category) => category.bytes > 0)
    .sort((a, b) => b.bytes - a.bytes || a.name.localeCompare(b.name));
  const inventoriedBytes = categories.reduce(
    (sum, category) => sum + category.bytes,
    0,
  );
  const disk = readDisk();
  const filesystemCounterComparable = disk.used >= inventoriedBytes * 0.95;
  const locations: StorageLocation[] = locationDefinitions
    .map((location) => ({ ...location, bytes: sizes.get(location.path) || 0 }))
    .filter((location) => location.bytes > 0)
    .sort((a, b) => b.bytes - a.bytes || a.name.localeCompare(b.name));

  return {
    updatedAt: new Date().toISOString(),
    scanIntervalMinutes: storageCacheMs / 60_000,
    inventory: { bytes: inventoriedBytes, categories, locations },
    filesystem: { ...disk, comparable: filesystemCounterComparable },
  };
}

function storageSnapshot() {
  if (!storageCached || Date.now() - storageCachedAt >= storageCacheMs) {
    storageCached = storageInventory();
...
function diskActivity(pid: number) {
  try {
    ...
    const now = Date.now();
    const current = {
      read: values.read_bytes || 0,
      write: values.write_bytes || 0,
      at: now,
    };
    const previous = processIo.get(pid);
    processIo.set(pid, current);
    if (!previous || now <= previous.at)
      return { ...current, readRate: 0, writeRate: 0 };
    const seconds = (now - previous.at) / 1000;
    return {
      ...current,
      readRate: Math.max(0, (current.read - previous.read) / seconds),
      writeRate: Math.max(0, (current.write - previous.write) / seconds),
    };
  } catch {
...
function parentPid(pid: number) {
  try {
    return number(
      readFileSync(`/proc/${pid}/status`, "utf8").match(/^PPid:\s+(\d+)/m)?.[1],
    );
  } catch {
    return 0;
  }
...
function applicationFor(
  ...
  const roots = new Map(
    serviceRoots().map((service) => [service.pid, service.name]),
  );
  let current = pid;
  ...
  for (let depth = 0; depth < 12 && current > 1; depth += 1) {
    ...
    if (service)
      return {
        application: applicationNames[service] || service,
        role: "Zo service",
      };
    current = parent;
    parent = parentPid(current);
  }
  if (name.includes("chrome") || args.includes("agent-browser"))
    return { application: "Zo Browser", role: "Browser session" };
  if (["zsh", "bash", "sh", "curl", "jq", "sleep"].includes(name))
    return { application: "Developer session", role: "Terminal command" };
  const fromWorkspace = workspaceApplication(workingDirectory(pid));
  if (fromWorkspace)
    return { application: fromWorkspace, role: "Workspace application" };
  if (name === "loki")
    return { application: "Zo Platform", role: "Log storage" };
  if (name === "promtail")
    return { application: "Zo Platform", role: "Log collection" };
  if (name.startsWith("next-server"))
    return { application: "Zo Platform", role: "Web application" };
  if (args.includes("mcpo"))
    return { application: "Zo Platform", role: "Tool gateway" };
  if (name === "sshd" || name === "frpc")
    return { application: "Zo Platform", role: "Secure connectivity" };
  if (workingDirectory(pid).startsWith("/__modal/"))
    return { application: "Zo Platform", role: "Workspace server" };
  if (name === "modal-daemon" || name === "dumb-init")
    return { application: "Zo Platform", role: "Container runtime" };
  if (name.includes("codex") || args.includes("/codex"))
    return { application: "Codex", role: "Development assistant" };
  if (name === "supervisord")
    return { application: "Zo Platform", role: "Service supervisor" };
...
function processes() {
  const output = new TextDecoder().decode(
    Bun.spawnSync([
      "ps",
      "-eo",
      "pid=,ppid=,pcpu=,pmem=,rss=,etimes=,nlwp=,comm=,args=",
      "--sort=-pcpu",
    ]).stdout,
  );
  return output
    ...
    .map((line) =>
      line
        .trim()
        .match(
          /^(\d+)\s+(\d+)\s+(\S+)\s+(\S+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\S+)\s+(.*)$/,
        ),
    )
    .filter((match): match is RegExpMatchArray => Boolean(match))
    .map((match) => {
...
function networkConnections(processList: ReturnType<typeof processes>) {
  ...
  const output = new TextDecoder().decode(
    Bun.spawnSync(["ss", "-tunapH"]).stdout,
  );
  return output
    .split("\n")
    .filter(Boolean)
    ...
    .sort(
      (a, b) =>
        (a.state === "LISTEN" ? -1 : 0) - (b.state === "LISTEN" ? -1 : 0) ||
        a.application.localeCompare(b.application),
    );
}
...
function collect() {
  ...
  const memory = {
    ...readMemory(),
    processResident: processList.reduce(
  ...
  const disk = {
    ...readDisk(),
    trackedRead: processList.reduce(
      (sum, process) => sum + process.diskRead,
      0,
    ),
    trackedWrite: processList.reduce(
      (sum, process) => sum + process.diskWrite,
      0,
    ),
    readRate: processList.reduce(
      (sum, process) => sum + process.diskReadRate,
      0,
    ),
    writeRate: processList.reduce(
      (sum, process) => sum + process.diskWriteRate,
      0,
    ),
  };
  const cpu = readCpu();
...
Bun.serve({
  ...
  async fetch(request) {
    ...
    if (url.pathname === `${basePath}/api/storage`) {
      return Response.json(storageSnapshot(), {
        headers: { "cache-control": "no-store" },
      });
    }
    if (url.pathname === `${basePath}/api/history`) {
      return Response.json(bandwidthHistory(), {
        headers: { "cache-control": "no-store" },
      });
    }
    if (
      url.pathname === `${basePath}/api/application-traffic` &&
      request.method === "POST"
    ) {
    ...
    if (url.pathname === `${basePath}/api/application-history`) {
      const application =
        url.searchParams.get("application")?.trim() || undefined;
      return Response.json(applicationTrafficHistory(application), {
        headers: { "cache-control": "no-store" },
      });
    }
